...
import { Plus, Flame, Leaf, Sun, Moon, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function GardenHUD({ onAddPlant }: GardenHUDProps) {
    ...
    <>
      ...
      >
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          {/* Logo / Title */}
          <div className={`${glassStyle} px-4 py-2 pointer-events-auto`}>
          ...
          <div className="flex items-center gap-3">
            ...
            <motion.div
              animate={{ 
                backgroundColor: lightingMode === 'golden' 
                  ? 'rgba(212, 175, 55, 0.2)' 
                  : 'rgba(74, 107, 138, 0.2)'
              }}
            ...
            >
              {lightingMode === 'golden' ? (
                ...
                <Moon className="w-4 h-4 text-[#4A6B8A]" />
              )}
              <span className={`text-xs font-ui ${textMuted} hidden md:inline`}>
                {lightingMode === 'golden' ? 'Golden Hour' : 'Twilight'}
              </span>
            </motion.div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={onAddPlant}
              disabled={plants.length >= 10}
              className={`
                relative group pointer-events-auto
                ${plants.length >= 10 ? 'opacity-50 cursor-not-allowed' : ''}
              `}
            >

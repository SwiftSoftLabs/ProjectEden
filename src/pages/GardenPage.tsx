...
export function GardenPage() {
  ...
  return (
    <div className={`w-full h-full overflow-hidden transition-colors duration-500 ${
      themeMode === 'dark' ? 'bg-[#050505]' : 'bg-[#F5F3EE]'
    }`}>
      {/* 3D Garden Scene */}
      ...
      {!showOnboarding && (
        <>
          <GardenHUD onAddPlant={() => setShowAddModal(true)} />
          ...
          <div className="fixed top-4 right-4 z-40 flex items-center gap-3">
            ...
            <GlassButton
              ...
              className={`flex items-center gap-2 ${
                themeMode === 'light' ? 'bg-black/10 text-[#050505]' : ''
              }`}
            >
              {themeMode === 'dark' ? (
            ...
            <div className="text-right">
              <p className={`font-ui text-xs ${
                themeMode === 'dark' ? 'text-marble/60' : 'text-[#050505]/60'
              }`}>{userEmail}</p>
            </div>
            <GlassButton
              variant="ghost"
